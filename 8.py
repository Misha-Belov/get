...
with open("./settings.txt", "r") as file_settings:
    settings_data = [float(num) for num in file_settings.read().split("\n")]
...
axes.set_xlabel("Время, c", fontsize = 10)
axes.set_ylabel("Напряжение, В", fontsize = 10)
axes.set_title("Процесс зарядки и разрядки конденастора в RC цепочке", fontsize = 20)
...
discharge_plot_line.set_label("V(t)")
...
axes.scatter(time_arr[volt_max_ind], volt_max, color='red')

axes.scatter(x = charge_time, y = 0.0, color='red')
axes.text(x=charge_time+0.1, y = 0.05, s=str(round(charge_time, 2)), fontsize = 12)

axes.scatter(x = 0.0, y = volt_max, color = 'red')
axes.text(x = 0.1, y = volt_max+0.05, s = str(round(volt_max, 2)), fontsize = 12)

axes.text(x = (charge_time/2-0.8), y = volt_max/2, s = ("Charge time: " + str(round(charge_time, 2)) + " s"), color = 'black', fontsize = 14)
axes.text(x = (charge_time+discharge_time/2-0.8), y = volt_max/2, s = ("Discharge time: " + str(round(discharge_time, 2)) + " s"), color = 'black', fontsize = 14)
